...
describe API::V2::Tickets do
	...
	context "unsuccessfuul requests" do
		...
		it "doesn't allow requests that pass an ivalid API key" do
			get url, {}, { "HTTP_AUTHORIZATION" => "Token token=notavalidkey" }
			expect(response.status).to eq	401
			expect(response.body).to include "Unauthenticated"
		end

		it "doesn't allow access to a ticket that the user doesn't have permission
			to read" do
